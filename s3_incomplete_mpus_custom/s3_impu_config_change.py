...
Description:
  Verify that S3 Bucket has a S3 lifecycle config for MPUS
...
Scenarios:
  ...
  Given: Has a lifecycle with 'AbortIncompleteMultipartUpload'
   Then: Return COMPLIANT

  Scenario 2:
  Given: NoSuchLifecycleConfiguration
...
def evaluate_compliance(event, configuration_item, valid_rule_parameters):
    ...
    try:
        ...
        result = s3.get_bucket_lifecycle_configuration(Bucket=s3_bucket_name) 
        Rules= result['Rules']
        
        if any('AbortIncompleteMultipartUpload' in d for d in Rules):
            #build_evaluation_from_config_item(configuration_item, 'COMPLIANT')
            bucketconfig = 'COMPLIANT'
        else: 
            #build_evaluation_from_config_item(configuration_item, 'NON_COMPLIANT')
            bucketconfig = 'NON_COMPLIANT'
    ...
    return build_evaluation_from_config_item(configuration_item, bucketconfig, f'The bucket {s3_bucket_name} is {bucketconfig}.')
